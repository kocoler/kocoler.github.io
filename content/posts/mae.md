...
子模块：
- 部署记录，也根据路由和服务分为两个部分
特定的 ID 记录用于 **回滚** 特定版本(集群粒度的控制)，并且 *实时（refresh status）* 显示每个部署的服务在集群上的状态
...
- 集群状态
    ...
    而这两个的创建时状态和上面一样
    这里主要涉及的是之后的状态：Pending, Healthy, Failed
...
- 回滚
    ...
    对于每一条 DeployRecord, 都可以回滚到相应的版本状态
    关于其他的 metadata, 统一 JSON 序列化，每次使用时再反序列化
